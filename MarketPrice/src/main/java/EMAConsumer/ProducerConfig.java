...
public class ProducerConfig {
     ...
     */
    private static final int SECONDS_TO_RUN_DEFAULT = 2; //20;
     ...
     */
    private static final int RECORDS_PER_SECOND_DEFAULT = 300;  //was set to 500

    /**
     * Change this to your stream name.
     */
    public static final String STREAM_NAME_DEFAULT = "StreamKPLMarketdataProd"; // //"MarketPrice_KPL"; //"StreamKPLMarketdataProd"; //"StreamKPLMarketdataProd";   "testStream" (for testing on dev)
    ...
    public ProducerConfig() {
        ...
        secondsToRun = SECONDS_TO_RUN_DEFAULT;
        recordsPerSecond = RECORDS_PER_SECOND_DEFAULT;
        dataSize = DATA_SIZE_DEFAULT;
        connections = 24;
        requestTimeout = 100000;  //60000
        bufferTime = 100; //1000;
        threadingModel = KinesisProducerConfiguration.ThreadingModel.POOLED.name(); //KinesisProducerConfiguration.ThreadingModel.PER_REQUEST.name();
        threadPoolSize = 10;
        aggregationEnabled = true; 
        aggregationMaxCount = Long.parseLong("4294967295");
        aggregationMaxSize = 51200; 
        recordTtl = 450000; //100000 
        rateLimit = 90;
        		
        ValidatorFactory factory = Validation.buildDefaultValidatorFactory();
        Validator validator = factory.getValidator();

        Set<ConstraintViolation<ProducerConfig>> violations = validator.validate(this);

        for (ConstraintViolation<ProducerConfig> violation : violations) {
    ...
    public KinesisProducerConfiguration transformToKinesisProducerConfiguration(){
        ...
        config.setRecordTtl(this.getRecordTtl());
        config.setRateLimit(this.getRateLimit());
